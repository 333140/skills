...
CITATION_PATTERNS = [
    ...
    # 模式3：《XX法》第X款（省略条号，较少见）
    (r'《([^》]+)》第(\d+)款',
     '款引用'),

    # 模式4：《XX法》第X项（省略条号款号，较少见）
    (r'《([^》]+)》第(\d+)项',
     '项引用'),

    # 模式5：《XX法》规定 / 《XX法》的相关规定
    (r'《([^》]+)》(?:的)?(?:相关)?规定',
     '泛引用'),

    # 模式6：《XX法》XX条款（如"第35条至第40条"）
    (r'《([^》]+)》第(\d+)条(?:第(\d+)款)?至第(\d+)条(?:第(\d+)款)?',
     '范围引用'),

    # 模式7：XX法第X条（无书名号）
    (r'(\w{2,10}(?:法|条例|规定|办法|决定|意见|通知|规则|标准|细则|解释))第(\d+)条(?:第(\d+)款)?(?:第(\d+)项)?',
     '无书名号引用'),
...
def group_by_law(citations: list[dict]) -> dict:
    ...
    for citation in citations:
        ...
        article_key = (article, citation.get('paragraph'), citation.get('item'))
        existing_keys = [
            (c.get('article'), c.get('paragraph'), c.get('item'))
